...
class FrontExpl
{
    public:
        FrontExpl()
        {
            ...
            tb3_1_FE_map_pub = nh.advertise<nav_msgs::OccupancyGrid>("edge/map", 1);
            region_map_pub = nh.advertise<nav_msgs::OccupancyGrid>("region/map", 1);
            map_0_sub = nh.subscribe<nav_msgs::OccupancyGrid>("tb3_1/map", 10, &FrontExpl::mapCallback, this);
        ...
        void main_loop()
        {
            ...
            while (ros::ok())
            {
                ...
                FE_tb3_1_map.header.frame_id = "tb3_1_FE_map";
                FE_tb3_1_map.info.resolution = map_0_msg.info.resolution;
                FE_tb3_1_map.info.width = map_0_msg.info.width;
                FE_tb3_1_map.info.height = map_0_msg.info.height;
                FE_tb3_1_map.info.origin.position.x = map_0_msg.info.origin.position.x;
                FE_tb3_1_map.info.origin.position.y = map_0_msg.info.origin.position.y ;
                FE_tb3_1_map.info.origin.position.z = map_0_msg.info.origin.position.z;
                FE_tb3_1_map.info.origin.orientation.w = map_0_msg.info.origin.orientation.w;
                FE_tb3_1_map.data = map_0_msg.data;
                ...
                int c = 0;
                int all_edges[FE_tb3_1_map.info.width * FE_tb3_1_map.info.height] = {};
                ...
                for (double x = FE_tb3_1_map.info.width + 1; x < (FE_tb3_1_map.info.width * FE_tb3_1_map.info.height) - FE_tb3_1_map.info.width; x++)
                {
                    if (FE_tb3_1_map.data[x] == -1)
                    {
                        // Store frontier edges
                        below_before_i = x - FE_tb3_1_map.info.width - 1;
                        below_i = x - FE_tb3_1_map.info.width;
                        below_after_i = x - FE_tb3_1_map.info.width + 1;
                        before_i = x-1;
                        after_i = x+1;
                        top_before_i = x + FE_tb3_1_map.info.width - 1;
                        top_i = x + FE_tb3_1_map.info.width;
                        top_after_i = x + FE_tb3_1_map.info.width + 1;

                        below_before = FE_tb3_1_map.data[below_before_i];
                        below = FE_tb3_1_map.data[below_i];
                        below_after = FE_tb3_1_map.data[below_after_i];
                        before = FE_tb3_1_map.data[before_i];
                        after = FE_tb3_1_map.data[after_i];
                        top_before = FE_tb3_1_map.data[top_before_i];
                        top = FE_tb3_1_map.data[top_i];
                        top_after = FE_tb3_1_map.data[top_after_i];
                ...
                for (int q=0; q < c-1; q++)
                {
                    ...
                    if ((next_value == 1) || (next_value == FE_tb3_1_map.info.width) || (next_value == FE_tb3_1_map.info.width -1) || (next_value == FE_tb3_1_map.info.width + 1))
                    {
                ...
                // Reset 
                c = 0;
                all_edges[FE_tb3_1_map.info.width * FE_tb3_1_map.info.height] = {};
                ...
                {
                    double dist;
                    point.x = (centroids[t] % FE_tb3_1_map.info.width)*FE_tb3_1_map.info.resolution + FE_tb3_1_map.info.origin.position.x;
                    point.y = floor(centroids[t]/FE_tb3_1_map.info.width)*FE_tb3_1_map.info.resolution + FE_tb3_1_map.info.origin.position.y;
                ...
                // Move to goal
                goal_init.target_pose.header.frame_id = "tb3_1/map"; // Needs to be AN ACTUAL FRAME
                goal_init.target_pose.header.stamp = ros::Time();
                goal_init.target_pose.pose.position.x = robot_pose_.position.x - 1.5;
                goal_init.target_pose.pose.position.y = robot_pose_.position.y;
    ...
    private:
        ros::NodeHandle nh;
        ros::Publisher tb3_1_FE_map_pub;
