...
class FrontExpl
{
    public:
        FrontExpl()
        {
            ...
            region1_map_pub = nh.advertise<nav_msgs::OccupancyGrid>("region_map_1", 1);
            // map_1_sub = nh.subscribe("tb3_1/map", 10, &FrontExpl::mapCallback, this);
            ...
            std::cout << "Initialized all the 1 things" << std::endl;
        }
        ...
        void mapCallback(const nav_msgs::OccupancyGrid & msg)
        {
            ...
            std::cout << "Got to map1 callback" << std::endl;
        ...
        void edge_index_to_point()
        {
            ...
            {
                point.x = (edge_vec.at(t) % FE1_map.info.width)*FE1_map.info.resolution + FE1_map.info.origin.position.x;
                point.y = floor(edge_vec.at(t) / FE1_map.info.width)*FE1_map.info.resolution + FE1_map.info.origin.position.y;
        ...
        void main_loop()
        {
            std::cout << "Entered the main loop for 1" << std::endl;
            ...
            while (ros::ok())
            {
                std::cout << "While ros okay for 1" << std::endl;
                // std::cout << "Can you call SpinOnce too many times?" << std::endl;

                ros::spinOnce();

                if (FE1_map.data.size()!=0 && start_flag == true)
                {
                    ...
                    find_regions();
                    // region1_map_pub.publish(region_map);
                    ...
                    std::cout << "Moiving to centroid " << centroid_Xpts.at(move_to_pt) << " , " <<  centroid_Ypts.at(move_to_pt) << std::endl;
    ...
    private:
        ros::NodeHandle nh;
        ros::Publisher FE1_map_pub, region1_map_pub, marker_pub;
        ros::Subscriber map_1_sub;
        ros::ServiceServer start_srv;
        // nav_msgs::OccupancyGrid FE1_map;
        nav_msgs::OccupancyGrid FE1_map, region_map;
        // visualization_msgs::MarkerArray centroid_arr;
